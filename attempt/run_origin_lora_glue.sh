...
target_task=(cola rte superglue-cb superglue-wsc-fixed mrpc stsb superglue-wic  superglue-multirc)

seed=4096
target_task=(scitail paws winogrande)
target_task=(searchqa)
...
#         output_dir="/home/linzhisheng/ATTEMPT/attempt/result_lora_8/$task/"$learning_rate"_"$per_device_train_batch_size"_"$CUDA_VISIBLE_DEVICES
...
target_task=(scitail paws winogrande cola rte superglue-cb superglue-wsc-fixed mrpc stsb superglue-wic superglue-boolq)
target_task=(cola rte superglue-cb superglue-wsc-fixed mrpc stsb superglue-wic superglue-boolq)
for task in ${target_task[@]}
do
    for learning_rate in ${lrs[@]}
    do
